...
# -------------------------------------------------------------------------------#
# Get list of classes for each set 获取每个集合的类列表
def classes_for_each_set(l_DIRS):
    '''
		-- l_DIRS : list of train, test and val. sets for the project.项目的训练、测试和调试设备清单。
	'''
...
# -------------------------------------------------------------------------------#
# Original data structuring for specific schema 特定模式的原始数据结构
def structure_origin_data(l_DIRS, IMAGE_FORMAT, POSITIVE_CLASS):
                ...
                for this_class, i in zip(unique_classes, range(0, len(unique_classes))):
                    ...
                    if len(unique_classes) > 1:
                        try:
                            ...
                            print('|||| Successfully created the directory for {} on {}'.format(this_class,
                                                                                                dir_for_class))
    ...
    # 12. Deleting Used Directories删除已使用目录
    time.sleep(5)
    os.chdir(cwd)
    ...
    # Averaging Dimmensions of Scanned Images扫描图像的平均尺寸
...
# -------------------------------------------------------------------------------#
# Plot Result Graph for Accuracy, Loss and Validation精度，损失和验证的结果图
